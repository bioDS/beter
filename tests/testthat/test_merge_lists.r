context("Merging structured lists")
...
test_that("Test additive merge with both existing parameters", {
    expect_equal(add(list1, list2), list("foo"=1, "bar"=c(2,3,4), "baz"=5))
    })


test_that("Test additive merge with a single missing parameter", {
...
test_that("Test additive merge with both missing parameters", {
    expect_equal(add(NULL, NULL), list())
    expect_equal(add(list(), list()), list())
    })


test_that("Test additive merge with a non-list argument", {
...
test_that("Test non-additive merge with both existing parameters", {
    expect_equal(merge(list1, list2), list("foo"=1, "bar"=4,"baz"=5))
    })


test_that("Test non-additive merge with a single missing parameter", {
...
test_that("Test non-additive merge with both missing parameters", {
    expect_equal(merge(NULL, NULL), list())
    expect_equal(merge(list(), list()), list())
    })


test_that("Test non-additive merge with a non-list argument", {
